...
config.google_api_key = ENV.fetch('REACT_APP_GOOGLE_CLIENT_ID')
  ...
  def show
    ...
    # resp = RestClient.get("https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=#{first}+NY&destinations=#{second}+NY&mode=walking&language=en-EN&key=#{config.google_api_key}");
    render json: resp
  end
end
